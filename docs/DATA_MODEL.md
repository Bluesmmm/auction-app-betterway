...
事务内锁定拍卖场次和当前最高出价；校验 1 分钟窗口；释放当前最高出价的积分冻结；清空当前最高价和最高出价人；不恢复历史被超越出价；写撤销、账本、审计和 outbox。
...
事务内锁定交易和相关账户；`release_to_buyer` 释放买家冻结并取消交易；`transfer_to_seller` 将买家冻结积分转给卖家并完成交易；`keep_frozen_for_platform_review` 继续冻结并进入平台复核；写原因、审计、流水和通知事件。
