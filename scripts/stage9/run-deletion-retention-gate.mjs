...
const schema = `stage9_deletion_retention_${Date.now()}_${process.pid}`;
const deletionRetentionGateTestFiles = [
  "apps/api/test/integration/child-data-retention.service.test.ts"
];

assertDeletionRetentionGateCoverage();

const commands = [];

if (await canConnectToDatabase()) {
  console.log("stage9 deletion-retention gate reusing existing database runtime");
} else {
...
commands.push(
  ...
  [
    "npm",
    ["test", "--", ...deletionRetentionGateTestFiles],
    {
      DATABASE_URL: databaseUrlForSchema(schema)
    }
  ]
...
try {
  ...
  console.log("stage9 deletion-retention gate passed");
} catch (error) {
  failure = error;
} finally {
...
function assertDeletionRetentionGateCoverage() {
  for (const file of deletionRetentionGateTestFiles) {
    if (!existsSync(file)) {
      throw new Error(
        `stage9 deletion-retention gate missing coverage file: ${file}`
      );
    }
  }
}
...
function assertStage9SchemaName(schemaName) {
  if (!/^stage9_deletion_retention_\d+_\d+$/.test(schemaName)) {
