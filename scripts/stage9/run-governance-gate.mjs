...
const schema = `stage9_governance_${Date.now()}_${process.pid}`;
const governanceGateTestFiles = [
...
assertGovernanceGateCoverage();

const commands = [];

if (await canConnectToDatabase()) {
  console.log("stage9 governance gate reusing existing database runtime");
} else {
...
try {
  ...
  console.log("stage9 governance gate passed");
} catch (error) {
  failure = error;
} finally {
...
function assertGovernanceGateCoverage() {
  for (const file of governanceGateTestFiles) {
    if (!existsSync(file)) {
      throw new Error(`stage9 governance gate missing coverage file: ${file}`);
    }
  }
}
...
function assertStage9SchemaName(schemaName) {
  if (!/^stage9_governance_\d+_\d+$/.test(schemaName)) {
