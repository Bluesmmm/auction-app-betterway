...
const schema = `stage9_outbox_worker_${Date.now()}_${process.pid}`;
const outboxWorkerGateTestFiles = [
...
assertOutboxWorkerGateCoverage();

const commands = [];

if (await canConnectToDatabase()) {
  console.log("stage9 outbox-worker gate reusing existing database runtime");
} else {
...
commands.push(
  ...
  [
    "npm",
    ["test", "--", ...outboxWorkerGateTestFiles],
    {
      DATABASE_URL: databaseUrlForSchema(schema)
    }
  ]
...
try {
  ...
  console.log("stage9 outbox-worker gate passed");
} catch (error) {
  failure = error;
} finally {
...
function assertOutboxWorkerGateCoverage() {
  for (const file of outboxWorkerGateTestFiles) {
    if (!existsSync(file)) {
      throw new Error(`stage9 outbox-worker gate missing coverage file: ${file}`);
    }
  }
}
...
function assertStage9SchemaName(schemaName) {
  if (!/^stage9_outbox_worker_\d+_\d+$/.test(schemaName)) {
