...
const schema = `stage9_privacy_content_${Date.now()}_${process.pid}`;
const privacyContentGateTestFiles = [
  "apps/api/test/contracts/content-provider-and-upload.test.ts",
  "apps/api/test/integration/stage3-content-review-flow.test.ts"
];

assertPrivacyContentGateCoverage();

const commands = [];

if (await canConnectToDatabase()) {
  console.log("stage9 privacy-content gate reusing existing database runtime");
} else {
...
commands.push(
  ...
  [
    "npm",
    ["test", "--", ...privacyContentGateTestFiles],
    {
      DATABASE_URL: databaseUrlForSchema(schema)
    }
  ]
...
try {
  ...
  console.log("stage9 privacy-content gate passed");
} catch (error) {
  failure = error;
} finally {
...
function assertPrivacyContentGateCoverage() {
  for (const file of privacyContentGateTestFiles) {
    if (!existsSync(file)) {
      throw new Error(`stage9 privacy-content gate missing coverage file: ${file}`);
    }
  }
}
...
function assertStage9SchemaName(schemaName) {
  if (!/^stage9_privacy_content_\d+_\d+$/.test(schemaName)) {
